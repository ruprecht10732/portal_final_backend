...
WITH search_query_base AS (
	SELECT
		websearch_to_tsquery('simple', rac_immutable_unaccent($3::text)) AS q_simple,
		websearch_to_tsquery('dutch', rac_immutable_unaccent($3::text)) AS q_dutch,
		(
			SELECT to_tsquery('simple', string_agg(lexeme || ':*', ' & '))
			FROM (
				SELECT regexp_replace(token, '[^[:alnum:]]', '', 'g') AS lexeme
				FROM unnest(tsvector_to_array(to_tsvector('simple', rac_immutable_unaccent($3::text)))) AS token
			) t
		...
		(
			...
			FROM (
				SELECT regexp_replace(token, '[^[:alnum:]]', '', 'g') AS lexeme
				FROM unnest(tsvector_to_array(to_tsvector('dutch', rac_immutable_unaccent($3::text)))) AS token
			) t
			WHERE length(lexeme) >= 3
		) AS q_dutch_prefix,
		rac_immutable_unaccent($3::text) AS q_text
),
search_query AS (
	SELECT
		q_simple,
...
func (q *Queries) GlobalSearch(ctx context.Context, arg GlobalSearchParams) ([]GlobalSearchRow, error) {
	rows, err := q.db.Query(ctx, globalSearch,
		arg.Types,
		arg.LimitCount,
		arg.QueryText,
		arg.OrgID,
