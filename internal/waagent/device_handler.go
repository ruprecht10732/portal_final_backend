...
func (h *DeviceHandler) Register(c *gin.Context) {
	...
	cfg, err := h.queries.UpsertAgentConfig(c.Request.Context(), waagentdb.UpsertAgentConfigParams{
		DeviceID:   deviceID,
	...
	if status.IsLoggedIn {
		...
		if infoErr == nil && info != nil {
			...
			if jid != "" {
				resp["accountJid"] = jid
				// Persist JID
				_, _ = h.queries.UpsertAgentConfig(c.Request.Context(), waagentdb.UpsertAgentConfigParams{
					DeviceID:   cfg.DeviceID,
