...
type senderTestConfigReader struct {
	config waagentdb.RacWhatsappAgentConfig
	err    error
}

func (r senderTestConfigReader) GetAgentConfig(context.Context) (waagentdb.RacWhatsappAgentConfig, error) {
	if r.err != nil {
		return waagentdb.RacWhatsappAgentConfig{}, r.err
	}
	return r.config, nil
}
...
func TestSenderSendReplyReturnsTransportError(t *testing.T) {
	...
	sender := newTestSender(transport, senderTestConfigReader{config: waagentdb.RacWhatsappAgentConfig{DeviceID: testSenderDeviceID}}, nil)
	err := sender.SendReply(context.Background(), uuid.New(), testSenderPhone, "Hallo")
...
func TestSenderSendReplyPersistsOutgoingMessage(t *testing.T) {
	...
	orgID := uuid.New()
	sender := newTestSender(transport, senderTestConfigReader{config: waagentdb.RacWhatsappAgentConfig{DeviceID: testSenderDeviceID}}, inboxWriter)
