...
func (s *Service) ListVatRatesWithFilters(ctx context.Context, tenantID uuid.UUID, req transport.ListVatRatesRequest) (transport.VatRateListResponse, error) {
	...
	return transport.VatRateListResponse{
		Items:      mapSlice(items, toVatRateResponse),
		Total:      total,
		Page:       page,
		PageSize:   pageSize,
...
func (s *Service) SeedDefaultVatRates(ctx context.Context, tenantID uuid.UUID) error {
	items, total, err := s.repo.ListVatRates(ctx, repository.ListVatRatesParams{
		OrganizationID: tenantID, Offset: 0, Limit: 1,
	})
	if err != nil {
		return err
	}
...
func (s *Service) ListProductsWithFilters(ctx context.Context, tenantID uuid.UUID, req transport.ListProductsRequest, vatRateID *uuid.UUID) (transport.ProductListResponse, error) {
	...
	return transport.ProductListResponse{
		Items:      mapSlice(items, toProductResponse),
		Total:      total,
		Page:       page,
		PageSize:   pageSize,
		TotalPages: calculateTotalPages(total, pageSize),
...
func (s *Service) CreateProduct(ctx context.Context, tenantID uuid.UUID, req transport.CreateProductRequest) (transport.ProductResponse, error) {
	...
	s.indexProductAsync(tenantID, product, "create")
	return toProductResponse(product), nil
}
...
func (s *Service) UpdateProduct(ctx context.Context, tenantID uuid.UUID, id uuid.UUID, req transport.UpdateProductRequest) (transport.ProductResponse, error) {
	...
	s.indexProductAsync(tenantID, product, "update")
	return toProductResponse(product), nil
}
...
// indexProductAsync pushes product data to Qdrant embeddings.
func (s *Service) indexProductAsync(tenantID uuid.UUID, product repository.Product, reason string) {
	...
	go func() {
		// Use a fresh timeout context for the background worker to prevent context leakage
		bgCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()

		resp, err := s.embeddingClient.AddDocuments(bgCtx, request)
		if err != nil {
			s.log.Error("catalog indexing failed", "error", err, "productId", product.ID, "reason", reason)
			return
		}
		s.log.Info("catalog indexed", "productId", product.ID, "documentsAdded", resp.DocumentsAdded, "reason", reason)
	}()
}
...
func (s *Service) ListProductMaterials(ctx context.Context, tenantID uuid.UUID, productID uuid.UUID) ([]transport.ProductResponse, error) {
	...
	return mapSlice(items, toProductResponse), nil
}
...
// =====================================================================
// Generics Helpers (Big O Optimizations)
// =====================================================================

// mapSlice uses Go 1.18 Generics to map any slice to another type (O(N) runtime, O(1) syntax).
...
// normalizePagination prevents DoS via unbounded queries.
